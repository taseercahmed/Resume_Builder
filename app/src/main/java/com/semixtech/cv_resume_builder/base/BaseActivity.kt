...
import com.semixtech.cv_resume_builder.helper.AppConstants
import com.semixtech.cv_resume_builder.helper.CustomDialog
import com.semixtech.cv_resume_builder.helper.SharedHelper
...
abstract class BaseActivity <DB : ViewDataBinding> : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (!TextUtils.isEmpty(SharedHelper.getKey(this, AppConstants.APP_LANGUAGE))) {
            val languageToLoad =
                SharedHelper.getKey(this, AppConstants.APP_LANGUAGE)   // your language
        ...
        mDialogBase = Dialog(context!!)
        mDialogBase!!.requestWindowFeature(Window.FEATURE_NO_TITLE)
        mDialogBase!!.setContentView(R.layout.custom_dialog_design)
        mDialogBase!!.setCancelable(false)
        mDialogBase!!.setCanceledOnTouchOutside(false)
        mDialogBase!!.getWindow()!!.setBackgroundDrawable(ColorDrawable(Color.TRANSPARENT))


        MyApplication.mInstance.setCurrentActivity(this)
...
//        callGrbadgeCollection()
        ...
        if (grantResults.size > 0) {
            ...
            if (requestCode == MY_REQUEST_CODE && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                ...
                if (ActivityCompat.checkSelfPermission(
                    ...
                    val dateArray = myDate.split(Pattern.quote(" ")).toTypedArray()
                    ...
                    val f = File(Environment.getExternalStorageDirectory(), photo)
            ...
            showErrorSnakbar("Unable to fetch result")
        }
    }
    ...
    private fun setImageUri(): Uri {
        val folder = File(context!!.filesDir, "abc")
        folder.mkdirs()

        val file = File(folder, "Image_Tmp${System.currentTimeMillis()}.jpg")
        if (file.exists())
            file.delete()
        file.mkdir()
        Log.i("TAG", "setImageUri: " + file.absolutePath)
        imageUri = FileProvider.getUriForFile(
            context!!,
            "com.invoiceslab.invoice.fileProvider",
            file
        )
        imgPath = file.absolutePath
        imageToUploadUri = imageUri
        Log.i("TAG", "setImageUri: " + imageUri)
        return imageUri!!
    }
...
//        }
    ...
    fun chooseFromCamera() {
        Log.i("TAG", "chooseFromCamera: 2222222222222")
        if (ActivityCompat.checkSelfPermission(
                this,
            ...
            if (ActivityCompat.checkSelfPermission(
                ...
                try {
                    ...
                    //  val f = File(applicationContext.getExternalFilesDir(null)!!, "temp.jpg")
                    val myDate =
                        DateFormat.getDateTimeInstance().format(Calendar.getInstance().time)
...
//                    )
                    ...
                    Log.i("TAG", "chooseFromCamera: error " + ex.toString())
                    ex.printStackTrace()
                }
            }
        }
    ...
    fun showCustomDialogueNewDesign(msg: String) {
        ...
        if (mDialogBase!!.isShowing) {
            UpdateCustomDialogueNewDesign(msg)
        } else {
          //  mDialogBase!!.messagetv!!.setText(msg)
            mDialogBase!!.setCancelable(false)
            mDialogBase!!.setCanceledOnTouchOutside(false)
            mDialogBase!!.show()
        }
